...
// Direct in-process discovery — works on Windows and Linux where the
// Electron sandbox does not block UDP multicast or LAN HTTP.
async function discoverDirect(timeout: number): Promise<DlnaDevice[]> {
    ...
    for (const addr of localAddresses) {
        ...
        socket.bind(0, addr, () => {
            try {
                ...
                // Not all interfaces support multicast
            }
            const send = () =>
    ...
    await new Promise<void>((r) => setTimeout(r, Math.max(timeout - 1000, 2000)));
...
// Child-process discovery — required on macOS where the Electron sandbox
// blocks UDP multicast and LAN HTTP from the main process.
// The inline script runs in a plain Node process outside the sandbox.
